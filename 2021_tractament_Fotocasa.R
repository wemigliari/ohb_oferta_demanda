...
mitjana_price_o_m <- aggregate(x = test111_21$price,    
                               by = list(foto_ofer_2021$property_id, 
                                         foto_ofer_2021$municipality, 
                                         foto_ofer_2021$mes),             
                               FUN = mean)                           
...
test111_21_pm_ud <- merge(x=test11a_21_fd, y=test11b_21_ld, by.x=c("property_id","municipality", "mes"), 
                          by.y=c("property_id","municipality", "mes"))
...
test111_21_pm_ud_dp <- merge(x=test111_21_pm_ud, y=test11b_21_dp, by.x=c("property_id","municipality", "mes"), 
                             by.y=c("property_id","municipality", "mes"))
...
test111_21$NOMMUNI<-gsub("L'Ametlla del Vallès","l'Ametlla del Vallès",as.character(test111_21$NOMMUNI))
...
test111_21$NOMMUNI[which(test111_21$NOMMUNI=="Alàs I Cerc")] <- "Alàs i Cerc"
...
test111_21$NOMMUNI[which(test111_21$NOMMUNI=="Les Franqueses del Vallès")] <- "les Franqueses del Vallès"
test111_21$NOMMUNI[which(test111_21$NOMMUNI=="Les Masies de Roda")] <- "les Masies de Roda"
test111_21$NOMMUNI[which(test111_21$NOMMUNI=="Les Masies de Voltregà")] <- "les Masies de Voltregà"
...
test_2021_f <- left_join(test_2021_f, preu_mitjana_mes_2021)%>%
  distinct(property_id, municipality, mes, .keep_all =TRUE)
test_2021_f <- left_join(test_2021_f, counting_muni_2021)%>%
  distinct(property_id, municipality, mes, .keep_all =TRUE)
...
na_date <- as.data.frame(ddply(test_2021_f, .(property_id), summarize, nNA=sum(is.na(date_posting_calcul))))
...
library(xlsx)
write_csv(test_2021_f, "/Users/wemigliari/Documents/Pós-Doutorado & Doutorado/Pós-Doc/Observatori_Metropolita/Dades/2021_Fotocasa_Oferta/tractament_foto_oferta_2021.csv")
