...
test_b2021 <- left_join(test_b2021, preu_mitjana_mes_b2021)%>%
  distinct(property_id, district, mes, .keep_all =TRUE)
test_b2021 <- left_join(test_b2021, counting_bcn_b2021)%>%
  distinct(property_id, district, mes, .keep_all =TRUE)
...
na_date <- as.data.frame(ddply(test_b2021, .(property_id), summarize, nNA=sum(is.na(date_posting_calcul))))
